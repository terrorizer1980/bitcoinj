...
public class ECKeyTest {
    ...
    public void base58Encoding() throws Exception {
        ...
        ECKey key = DumpedPrivateKey.fromBase58(TestBtcNet3Params.get(), privkey).getKey();
        assertEquals(privkey, key.getPrivateKeyEncoded(TestBtcNet3Params.get()).toString());
        assertEquals(addr, key.toAddress(TestBtcNet3Params.get()).toString());
    }
    ...
    public void base58Encoding_stress() throws Exception {
        ...
        for (int i = 0 ; i < 20 ; i++) {
            ECKey key = new ECKey();
            ECKey key1 = DumpedPrivateKey.fromBase58(TestBtcNet3Params.get(),
                    key.getPrivateKeyEncoded(TestBtcNet3Params.get()).toString()).getKey();
    ...
    public void signTextMessage() throws Exception {
        ...
        log.info("Message signed with " + key.toAddress(MainBtcNetParams.get()) + ": " + signatureBase64);
    ...
    public void verifyMessage() throws Exception {
        ...
        Address expectedAddress = Address.fromBase58(MainBtcNetParams.get(), "14YPSNPi6NSXnUxtPAsyJSuw3pv7AU3Cag");
        ECKey key = ECKey.signedMessageToKey(message, sigBase64);
        Address gotAddress = key.toAddress(MainBtcNetParams.get());
