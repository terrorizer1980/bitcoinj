 ...
 *
 * <ul>
 * <li>{@code bitcoincash:<address>}</li>
 * <li>{@code bitcoincash:<address>?<name1>=<value1>&<name2>=<value2>} with multiple
...
public class BitcoinCashURI {
    ...
    /**
     * Constructs a new BitcoinCashURI from the given string. Can be for any network.
     *
    ...
    /**
     * Constructs a new object by trying to parse the input as a valid Bitcoin Cash URI.
     *
     ...
     *
     * @throws BitcoinURIParseException If the input fails Bitcoin Cash URI syntax and semantic checks.
     */
    public BitcoinCashURI(@Nullable NetworkParameters params, String input) throws BitcoinURIParseException {
        ...
        String scheme = null == params
            ? BitcoinCashMainNetParams.URI_SCHEME
            : params.getUriScheme();
        ...
        // URI is formed as  bitcoincash:<address>?<query parameters>
        // blockchain.info generates URIs of non-BIP compliant form bitcoincash://address?....
        // We support both until Ben fixes his code.
        
        // Remove the bitcoin cash scheme.
        // (Note: getSchemeSpecificPart() is not used as it unescapes the label and parse then fails.
        // For instance with : bitcoincash:129mVqKUmJ9uwPxKJBnNdABbuaaNfho4Ha?amount=0.06&label=Tom%20%26%20Jerry
        ...
        if (!addressToken.isEmpty()) {
            // Attempt to parse the addressToken as a Bitcoin cash address for this network
            try {
    ...
    private void parseParameters(@Nullable NetworkParameters params, String addressToken, String[] nameValuePairTokens) throws BitcoinURIParseException {
        ...
        for (String nameValuePairToken : nameValuePairTokens) {
            ...
            if (sepIndex == -1)
                throw new BitcoinURIParseException("Malformed Bitcoin Cash URI - no separator in '" +
                        nameValuePairToken + "'");
            if (sepIndex == 0)
                throw new BitcoinURIParseException("Malformed Bitcoin Cash URI - empty name '" +
                        nameValuePairToken + "'");
    ...
    /**
     * The Bitcoin Cash Address from the URI, if one was present. It's possible to have Bitcoin Cash URI's with no address if a
    ...
    /**
     * Simple Bitcoin Cash URI builder using known good fields.
     *
     * @param address The Bitcoin Cash address
     * @param amount The amount
     * @param label A label
     * @param message A message
     * @return A String containing the Bitcoin Cash URI
     */
    public static String convertToBitcoinCashURI(Address address, Coin amount,
                                             String label, String message) {
        return convertToBitcoinCashURI(address.getParameters(), address.toString(), amount, label, message);
    }
     ...
     * @return A String containing the Bitcoin Cash URI
     */
    public static String convertToBitcoinCashURI(NetworkParameters params,
